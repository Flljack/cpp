...
void update(const sf::Vector2f &mousePosition, sf::ConvexShape &pointer, const float deltaTime)
{
    const sf::Vector2f delta = mousePosition - pointer.getPosition();
    const float angleMaxSpeed = 15 * deltaTime;
    ...
    if (angle != pointer.getRotation())
    {
        if (angle > pointer.getRotation())
        {
            if ((angle - 180) > pointer.getRotation())
                pointer.setRotation(pointer.getRotation() - std::min(angleMaxSpeed, rotation));
            else
                pointer.setRotation(pointer.getRotation() + std::min(angleMaxSpeed, rotation));
        }
        else
